...
class Tabellone:
    ...
    def __init__(self):
        ...
        Si procede in maniera analoga per le altre cartelle del tabellone
        '''
        ...
        '''La coppia 1 forma le prime due cartelle in alto del tabellone'''
        coppia1=np.zeros((3,10))
        coppia1[0]=c+c1 #1,2,3,4,5 e 6,7,8,9,10
        coppia1[1]=c2+c3#11,12,13,14,15 e 16,17,18,19,20
        coppia1[2]=c4+c5#21,22,23,24,25 e 26,27,28,29,30
        '''La coppia 2 forma le cartelle centrali del tabellone'''
        coppia2=np.zeros((3,10))
        coppia2[0]=c6+c7
        coppia2[1]=c8+c9
        coppia2[2]=c10+c11
        '''La coppia 3 forma le cartelle restanti del tabellone'''
        ...
        Viene fatta una suddivisione tra cartellone sinistro e destro'''
        ...
        '''Self.cartellone_dx_sx contiene tutte le cartelle del tabellone. Gli indici associati a ciascuna tabella sono i seguenti:
        TAB_SX dall'alto verso il basso:0,1,2
        TAB_DX dall'alto verso il basso:3,4,5'''
        self.cartellone_dx_sx=self.cartellone_sx+self.cartellone_dx

        '''si realizzano tre liste contenenti ciascuna 2 cartelle per permettere una visualizzazione migliore'''
    ...
    '''metodo che permette la visualizzazione di un singolo elemento del tabellone noti gli indici di posizione
        ...
        - pos_cartella indica la matrice(cartella) a cui ci riferiamo 
    ...
    '''effettua la stampa del tabellone, li dove il numero nella cartella è 0 si inserisce uno spazio vuoto.
    Se invece è stato estratto un numero contenuto nella cartella, per tenerne conto andremo a inserire un *.
    Se non si verificano le situazioni precedenti vengono visualizzati i numeri nella cartella'''   
    ...
    ''' Restituisce la singola cartella del tabellone sinistro'''
    def singola_cartella_tab_sx(self,i):
        return self.cartellone_sx[i]

    '''Analoga al precedente, solamente viene tenuto conto del tab.destro'''
    def singola_cartella_tab_dx(self,i):
        return self.cartellone_dx[i]

    "Nota la cartella iesima del tabellone mi restituisce la cartella iesima che voglio visualizzare"
    def singola_cartella_tab_dx_sx(self,i):
        return self.cartellone_dx_sx[i]

    '''Noto il numero n estratto,grazie a dei cicli sulle righe e colonne della matrice, e sul numero totale di cartelle, riesco a trovare
    gli indici k,i,j, associati rispettivamente alla cartella e alle posizioni nella cartella in cui si trova quel numero. Ad esempio il numero 48 si troverà in posizione (4,1,2)'''
            ...
            for j in range(5):
                for k in range(6):
                    if self.singola_cartella_tab_dx_sx(k)[i][j]==n:
                        return k,i,j
    '''Sostituisco -1 nella cartella k alla posizione i,j'''
    ...
    '''nota la cartella k e le posizioni i j della cartella, sostituisco il valore -1 '''
    def sostituisci_numero_tabellone(self,k,i,j):
        self.cartellone[k][i][j]=-1

    '''Nota la cartella k del tabellone e la riga iesima, scorrendo le colonne della cartella e facendo un controllo sulle volte in cui appare il valore -1,
    grazie a un contatore riesco a capire effettivamente quanti numeri sono stati estratti su quella riga. Grazie a questo potremmo successivamente effettuare una verifica
    delle vincite '''
